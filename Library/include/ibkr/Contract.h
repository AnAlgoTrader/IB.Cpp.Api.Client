...
{
	Contract()
		: conId(0), strike(0), includeExpired(false), comboLegs(NULL), deltaNeutralContract(NULL)
	{
	}
...
{
	ContractDetails()
		: minTick(0), priceMagnifier(0), underConId(0), evMultiplier(0), minSize(UNSET_DECIMAL), sizeIncrement(UNSET_DECIMAL), suggestedSizeIncrement(UNSET_DECIMAL), callable(false), putable(false), coupon(0), convertible(false), nextOptionPartial(false)
	{
		aggGroup = 0;
	}
