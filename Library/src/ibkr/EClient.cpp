...
EClient::EClient(EWrapper *ptr, ETransport *pTransport) :
		m_pEWrapper(ptr), m_transport(pTransport), m_clientId(-1), m_connState(
				CS_DISCONNECTED), m_extraAuth(false), m_serverVersion(0), m_useV100Plus(
				true)
{
	m_port = 0;
}
...
void EClient::setConnectOptions(const std::string &connectOptions)
{
	if (isSocketOK())
	{
		m_pEWrapper->error(NO_VALID_ID, ALREADY_CONNECTED.code(),
...
void EClient::reqMktData(TickerId tickerId, const Contract &contract,
		const std::string &genericTicks, bool snapshot, bool regulatorySnaphsot,
		const TagValueListSPtr &mktDataOptions)
{
	...
	try
	{
		...
		{
			const Contract::ComboLegList *const comboLegs =
					contract.comboLegs.get();
	...
	{
		m_pEWrapper->error(tickerId, ex.error().code(),
...
void EClient::reqMktDepth(TickerId tickerId, const Contract &contract,
		int numRows, bool isSmartDepth, const TagValueListSPtr &mktDepthOptions)
{
	// not connected?
	if (!isConnected())
	{
...
void EClient::reqScannerParameters()
{
	...
	if (!isConnected())
	{
		m_pEWrapper->error(NO_VALID_ID, NOT_CONNECTED.code(),
				NOT_CONNECTED.msg(), "");
		return;
	}
		...
		const TagValueListSPtr &fundamentalDataOptions)
{
	...
	{
		m_pEWrapper->error(reqId, ex.error().code(),
				ex.error().msg() + ex.text(), "");
		return;
	}
...
void EClient::calculateOptionPrice(TickerId reqId, const Contract &contract,
...
void EClient::placeOrder(OrderId id, const Contract &contract,
		const Order &order)
{
	// not connected?
	if (!isConnected())
	...
	if (m_serverVersion < MIN_SERVER_VER_SCALE_ORDERS3)
	{
		if (order.scalePriceIncrement
				> 0&& order.scalePriceIncrement != UNSET_DOUBLE)
		{
			...
			&& contract.secType == "BAG")
	{
		...
		const int orderComboLegsCount =
				orderComboLegs ? orderComboLegs->size() : 0;
	...
	{
		if (!order.scaleTable.empty() || !order.activeStartTime.empty()
				|| !order.activeStopTime.empty())
		{
			m_pEWrapper->error(id, UPDATE_TWS.code(),
					UPDATE_TWS.msg()
	...
	{
		if (!order.softDollarTier.name().empty()
				|| !order.softDollarTier.val().empty())
		{
			m_pEWrapper->error(id, UPDATE_TWS.code(),
	...
	if (m_serverVersion
			< MIN_SERVER_VER_DURATION&& order.duration != UNSET_INTEGER)
	{
		m_pEWrapper->error(id, UPDATE_TWS.code(),
	...
	if (m_serverVersion
			< MIN_SERVER_VER_POST_TO_ATS&& order.postToAts != UNSET_INTEGER)
	{
		m_pEWrapper->error(id, UPDATE_TWS.code(),
	...
	if (m_serverVersion < MIN_SERVER_VER_ADVANCED_ORDER_REJECT
			&& !order.advancedErrorOverride.empty())
	{
		m_pEWrapper->error(id, UPDATE_TWS.code(),
				UPDATE_TWS.msg()
	...
	try
	{
				...
				&& contract.secType == "BAG")
		{
			...
			if (orderComboLegsCount > 0)
			{
				for (int i = 0; i < orderComboLegsCount; ++i)
				{
					const OrderComboLeg *orderComboLeg =
							((*orderComboLegs)[i]).get();
					assert(orderComboLeg);
				...
				&& contract.secType == "BAG")
		{
			...
			const int smartComboRoutingParamsCount =
					smartComboRoutingParams ?
							smartComboRoutingParams->size() : 0;
		...
		if (m_serverVersion >= MIN_SERVER_VER_DELTA_NEUTRAL_CONID
				&& !order.deltaNeutralOrderType.empty())
		{
		...
		if (m_serverVersion
				>= MIN_SERVER_VER_SCALE_ORDERS3&& order.scalePriceIncrement > 0.0 && order.scalePriceIncrement != UNSET_DOUBLE)
		{
...
void EClient::reqAccountUpdates(bool subscribe, const std::string &acctCode)
{
	...
	{
		m_pEWrapper->error(NO_VALID_ID, ex.error().code(),
				ex.error().msg() + ex.text(), "");
		return;
	}
...
void EClient::replaceFA(int reqId, faDataType pFaDataType,
...
void EClient::reqAccountSummary(int reqId, const std::string &groupName,
...
void EClient::verifyAndAuthRequest(const std::string &apiName,
		const std::string &apiVersion, const std::string &opaqueIsvKey)
{
	// not connected?
	if (!isConnected())
	{
...
void EClient::reqPositionsMulti(int reqId, const std::string &account,
...
void EClient::reqAccountUpdatesMulti(int reqId, const std::string &account,
...
void EClient::reqSecDefOptParams(int reqId, const std::string &underlyingSymbol,
...
void EClient::reqNewsArticle(int requestId, const std::string &providerCode,
		...
		const TagValueListSPtr &newsArticleOptions)
{
	...
	{
		m_pEWrapper->error(requestId, ex.error().code(),
				ex.error().msg() + ex.text(), "");
		return;
	}
...
void EClient::reqHistoricalNews(int requestId, int conId,
		const std::string &providerCodes, const std::string &startDateTime,
		const std::string &endDateTime, int totalResults,
...
void EClient::reqHeadTimestamp(int tickerId, const Contract &contract,
...
void EClient::reqHistogramData(int reqId, const Contract &contract, bool useRTH,
...
void EClient::reqPnL(int reqId, const std::string &account,
		const std::string &modelCode)
{
	if (!isConnected())
	{
...
void EClient::reqPnLSingle(int reqId, const std::string &account,
...
void EClient::reqHistoricalTicks(int reqId, const Contract &contract,
		const std::string &startDateTime, const std::string &endDateTime,
		int numberOfTicks, const std::string &whatToShow, int useRth,
		bool ignoreSize, const TagValueListSPtr &miscOptions)
{
...
void EClient::reqTickByTickData(int reqId, const Contract &contract,
...
void EClient::reqWshEventData(int reqId, const WshEventData &wshEventData)
{
	...
	{
		if (!wshEventData.filter.empty() || wshEventData.fillWatchlist
