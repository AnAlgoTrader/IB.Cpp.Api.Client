...
bool EClientSocket::eConnect(const char *host, int port, int clientId,
		bool extraAuth)
{
	if (m_fd == -2)
	{
		getWrapper()->error(NO_VALID_ID, FAIL_CREATE_SOCK.code(),
	...
	{
		errno = EISCONN;
		getWrapper()->error(NO_VALID_ID, ALREADY_CONNECTED.code(),
...
bool EClientSocket::eConnectImpl(int clientId, bool extraAuth,
...
void EClientSocket::serverVersion(int version, const char *time)
{
	...
	if (usingV100Plus() ?
			(m_serverVersion < MIN_CLIENT_VER
					|| m_serverVersion > MAX_CLIENT_VER) :
			m_serverVersion < MIN_SERVER_VER_SUPPORTED)
	{
		eDisconnect();
		getWrapper()->error(NO_VALID_ID, UNSUPPORTED_VERSION.code(),
