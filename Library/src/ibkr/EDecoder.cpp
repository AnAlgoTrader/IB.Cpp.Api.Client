		...
		const char *endPtr)
{
	...
	if (version >= 6 || tickTypeInt == MODEL_OPTION
			|| tickTypeInt == DELAYED_MODEL_OPTION_COMPUTATION)
	...
	m_pEWrapper->tickOptionComputation(tickerId, (TickType) tickTypeInt,
			tickAttrib, impliedVol, delta, optPrice, pvDividend, gamma, vega,
			theta, undPrice);
...
const char* EDecoder::processOrderStatusMsg(const char *ptr, const char *endPtr)
{
	...
	m_pEWrapper->orderStatus(orderId, status, filled, remaining, avgFillPrice,
...
const char* EDecoder::processOpenOrderMsg(const char *ptr, const char *endPtr)
{
	...
	EOrderDecoder eOrderDecoder(&contract, &order, &orderState, version,
			m_serverVersion);
	...
	// read order fields
	bool success = eOrderDecoder.decodeAction(ptr, endPtr)
		...
		const char *endPtr)
{
	...
	m_pEWrapper->updatePortfolio(contract, position, marketPrice, marketValue,
		...
		const char *endPtr)
{
	...
	if (m_serverVersion >= MIN_SERVER_VER_MD_SIZE_MULTIPLIER
			&& m_serverVersion < MIN_SERVER_VER_SIZE_RULES)
	{
		...
		const char *endPtr)
{
	...
	m_pEWrapper->realtimeBar(reqId, time, open, high, low, close, volume, wap,
			count);
		...
		const char *endPtr)
{
	...
	m_pEWrapper->positionMulti(reqId, account, modelCode, contract, position,
			avgCost);
		...
		const char *endPtr)
{
	...
	m_pEWrapper->accountUpdateMulti(reqId, account, modelCode, key, value,
			currency);
...
const char* EDecoder::processSecurityDefinitionOptionalParameterMsg(
		const char *ptr, const char *endPtr)
{
	...
	m_pEWrapper->securityDefinitionOptionalParameter(reqId, exchange,
			underlyingConId, tradingClass, multiplier, expirations, strikes);

	return ptr;
}

const char* EDecoder::processSecurityDefinitionOptionalParameterEndMsg(
		const char *ptr, const char *endPtr)
{
	int reqId;
	...
	if (nDepthMktDataDescriptions > 0)
	{
		...
		for (int i = 0; i < nDepthMktDataDescriptions; ++i)
		{
			...
			else
			{
				...
				bool isL2;
				DECODE_FIELD(isL2);
				depthMktDataDescriptions[i].serviceDataType =
						isL2 ? "Deep2" : "Deep";
			}
		}
	}
...
const char* EDecoder::processTickNewsMsg(const char *ptr, const char *endPtr)
{
	...
	m_pEWrapper->tickNews(tickerId, timeStamp, providerCode, articleId,
			headline, extraData);
		...
		const char *endPtr)
{
	...
	m_pEWrapper->historicalNews(requestId, time, providerCode, articleId,
			headline);
		...
		const char *endPtr)
{
	...
	m_pEWrapper->tickReqParams(tickerId, minTick, bboExchange,
			snapshotPermissions);
...
const char* EDecoder::processPnLSingleMsg(const char *ptr, const char *endPtr)
{
	...
	m_pEWrapper->pnlSingle(reqId, pos, dailyPnL, unrealizedPnL, realizedPnL,
			value);
...
const char* EDecoder::decodeTick(HistoricalTick &tick, const char *ptr,
		const char *endPtr)
{
	int nope;
...
const char* EDecoder::decodeTick(HistoricalTickBidAsk &tick, const char *ptr,
		const char *endPtr)
{
	DECODE_FIELD(tick.time);
	int attrMask;
...
const char* EDecoder::decodeTick(HistoricalTickLast &tick, const char *ptr,
		const char *endPtr)
{
	DECODE_FIELD(tick.time);
	int attrMask;
		...
		const char *endPtr)
{
	...
	{ // Last/AllLast
		...
		m_pEWrapper->tickByTickAllLast(reqId, tickType, time, price, size,
	...
	{ // BidAsk
		...
		m_pEWrapper->tickByTickBidAsk(reqId, time, bidPrice, askPrice, bidSize,
		...
		const char *endPtr)
{
	...
	Contract contract;
	OrderState orderState;
	EOrderDecoder eOrderDecoder(&contract, &order, &orderState, UNSET_INTEGER,
			m_serverVersion);
...
const char* EDecoder::processHistoricalSchedule(const char *ptr,
		const char *endPtr)
{
	...
	m_pEWrapper->historicalSchedule(reqId, startDateTime, endDateTime, timeZone,
			sessions);
...
int EDecoder::parseAndProcessMsg(const char *&beginPtr, const char *endPtr)
{
	...
	try
	{
		...
		case COMPLETED_ORDERS_END:
			ptr = processCompletedOrdersEndMsg(ptr);
			break;
...
bool EDecoder::DecodeField(bool &boolValue, const char *&ptr,
		const char *endPtr)
{
	int intValue;
...
bool EDecoder::DecodeFieldTime(time_t &time_tValue, const char *&ptr,
		const char *endPtr)
{
	if (!CheckOffset(ptr, endPtr))
		return false;
...
bool EDecoder::DecodeField(long long &longLongValue, const char *&ptr,
		const char *endPtr)
{
	if (!CheckOffset(ptr, endPtr))
		return false;
...
bool EDecoder::DecodeField(long &longValue, const char *&ptr,
		const char *endPtr)
{
	if (!CheckOffset(ptr, endPtr))
		return false;
...
bool EDecoder::DecodeField(double &doubleValue, const char *&ptr,
		const char *endPtr)
{
	if (!CheckOffset(ptr, endPtr))
		return false;
...
bool EDecoder::DecodeField(std::string &stringValue, const char *&ptr,
		const char *endPtr)
{
	if (!CheckOffset(ptr, endPtr))
		return false;
...
bool EDecoder::DecodeField(char &charValue, const char *&ptr,
		const char *endPtr)
{
	if (!CheckOffset(ptr, endPtr))
		return false;
...
bool EDecoder::DecodeField(Decimal &decimalValue, const char *&ptr,
		const char *endPtr)
{
	if (!CheckOffset(ptr, endPtr))
		return false;
...
bool EDecoder::DecodeFieldMax(int &intValue, const char *&ptr,
		const char *endPtr)
{
...
bool EDecoder::DecodeFieldMax(long &longValue, const char *&ptr,
		const char *endPtr)
{
	int intValue;
...
bool EDecoder::DecodeFieldMax(double &doubleValue, const char *&ptr,
		const char *endPtr)
{
	std::string stringValue;
	if (!DecodeField(stringValue, ptr, endPtr))
		return false;
	doubleValue =
			stringValue.empty() ? UNSET_DOUBLE : atof(stringValue.c_str());
	return true;
}

const char* EDecoder::decodeLastTradeDate(const char *ptr, const char *endPtr,
		ContractDetails &contract, bool isBond)
{
