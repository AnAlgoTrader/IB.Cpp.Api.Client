...
EOrderDecoder::EOrderDecoder(Contract *contract, Order *order,
		OrderState *orderState, int version, int serverVersion)
{
	m_contract = contract;
	m_order = order;
...
bool EOrderDecoder::decodeVolOrderParams(const char *&ptr, const char *endPtr,
...
bool EOrderDecoder::decodeScaleOrderParams(const char *&ptr, const char *endPtr)
{
	...
	if (m_version
			>= 28&& m_order->scalePriceIncrement > 0.0 && m_order->scalePriceIncrement != UNSET_DOUBLE)
	{
	...
	if (m_serverVersion >= MIN_SERVER_VER_PEGGED_TO_BENCHMARK)
	{
		...
		if (conditionsSize > 0)
		{
			for (; conditionsSize; conditionsSize--)
			{
				...
				std::shared_ptr<OrderCondition> item =
						std::shared_ptr<OrderCondition>(
								OrderCondition::create(
										(OrderCondition::OrderConditionType) conditionType));
...
bool EOrderDecoder::decodeAutoCancelParent(const char *&ptr, const char *endPtr,
		int minVersionAutoCancelParent)
{
...
bool EOrderDecoder::decodePegBestPegMidOrderAttributes(const char *&ptr,
		const char *endPtr)
{
