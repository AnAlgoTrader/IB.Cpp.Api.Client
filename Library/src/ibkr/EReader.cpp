...
void EReader::readToQueue()
{
	...
	while (m_isAlive)
	{
		if (m_buf.size() == 0 && !processNonBlockingSelect()
				&& m_pClientSocket->isSocketOK())
			continue;
...
bool EReader::processNonBlockingSelect()
{
	...
	if (m_pClientSocket->fd() >= 0)
	{
		...
		int ret = select(m_pClientSocket->fd() + 1, &readSet, &writeSet,
				&errorSet, &tval);
...
EMessage* EReader::readSingleMsg()
{
	...
	else
	{
		...
		while (msgSize == 0)
		{
			...
			msgSize = EDecoder(m_pClientSocket->EClient::serverVersion(),
					&defaultWrapper).parseAndProcessMsg(pBegin, pEnd);
		}
		...
		if (m_buf.size() < IN_BUF_SIZE_DEFAULT
				&& m_buf.capacity() > IN_BUF_SIZE_DEFAULT)
		{
